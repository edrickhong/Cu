...
void VPushBackVertexSpecDesc(VGraphicsPipelineSpec* spec,u32 bind_no,u32 unit_size,
    ...
    spec->desc_container[spec->desc_count] = {bind_no,unit_size,inputrate};
    spec->desc_count++;
}

void VPushBackVertexSpecAttrib(VGraphicsPipelineSpec* spec,u32 bind_no,VkFormat format,
                               u32 attrib_size){
    
    _kill("exceeded bindingspec types\n",
          spec->attrib_count >= 10);
    
    spec->attrib_container[spec->attrib_count] =
    {spec->attrib_count,bind_no,format,spec->offset};
    
    spec->offset += attrib_size;
...
void VPushBackInstanceSpecAttrib(VGraphicsPipelineSpec* spec,u32 bind_no,VkFormat format,
    ...
    spec->attrib_container[spec->attrib_count] =
    {spec->attrib_count,bind_no,format,spec->inst_offset};
    
    spec->inst_offset += attrib_size;
...
void VPushBackShaderPipelineSpec(VGraphicsPipelineSpec* spec,void* shader_data,
    ...
    _kill("exceeded bindingspec types\n",
          spec->shaderdata_count >= 6);
    
    spec->shaderdata_array[spec->shaderdata_count] = shader_data;
    spec->shaderdatasize_array[spec->shaderdata_count] = shader_size;
    spec->shaderstage_array[spec->shaderdata_count] = stage;
    spec->shaderspecialization_array[spec->shaderdata_count] = specialization;
    
    spec->shaderdata_count++;
}



_ainline s8* GetData(VGraphicsPipelineSpec* spec,u32 size){
    _kill("not enough space\n",(u64)(spec->cur + size - spec->buffer) > (u64)sizeof(spec->buffer));
    auto data = spec->cur;
    spec->cur += size;
    return data;
} 
...
void VGenerateGraphicsPipelineSpec(VGraphicsPipelineSpec* spec,
                                   ...
                                   VkRenderPass renderpass,u32 colorattachment_count,
                                   u32 subpass_index,VkPipelineCreateFlags flags,
    ...
    VSetFixedViewportGraphicsPipelineSpec(spec,width,height);
    
    VSetMultisampleGraphicsPipelineSpec(spec,VK_SAMPLE_COUNT_1_BIT,VK_FALSE,1.0f,0,
                                        VK_FALSE,VK_FALSE);
    
    VSetDepthStencilGraphicsPipelineSpec(spec,VK_FALSE,VK_FALSE,VK_COMPARE_OP_NEVER,
                                         VK_FALSE);
    
    VkPipelineColorBlendAttachmentState attachments[16] = {};
    
    _kill("we do not support this many color attachments\n",
          colorattachment_count > _arraycount(attachments));
    ...
    VSetColorBlend(spec,&attachments[0],colorattachment_count);
}


void VSetFixedViewportGraphicsPipelineSpec(VGraphicsPipelineSpec* spec,VkViewport* viewport,
                                           u32 viewport_count,VkRect2D* scissor,u32 scissor_count){
    
    VkViewport* viewport_array = (VkViewport*)GetData(spec,sizeof(VkViewport) * viewport_count);
    
    VkRect2D* scissor_array = (VkRect2D*)GetData(spec,sizeof(VkRect2D) * scissor_count);
    
    memcpy(viewport_array,viewport,sizeof(VkViewport) * viewport_count);
    memcpy(scissor_array,scissor,sizeof(VkRect2D) * scissor_count);
    
    
    spec->viewport.sType = VK_STRUCTURE_TYPE_PIPELINE_VIEWPORT_STATE_CREATE_INFO;
    spec->viewport.pNext = 0;
    spec->viewport.flags = 0;
    spec->viewport.viewportCount = viewport_count;
    spec->viewport.pViewports = viewport_array;
    spec->viewport.scissorCount = scissor_count;
    spec->viewport.pScissors = scissor_array;
}
...
void VSetFixedViewportGraphicsPipelineSpec(VGraphicsPipelineSpec* spec,
    ...
    viewport = {0.0f,0.0f,(f32)width,(f32)height,0.0f,1.0f};
...
void VSetDepthStencilGraphicsPipelineSpec(VGraphicsPipelineSpec* spec,VkBool32 depthtest_enable,
                                          VkBool32 depthwrite_enable,VkCompareOp depthtest_op,
                                          VkBool32 depthboundstest_enable,
                                          f32 min_depthbounds,f32 max_depthbounds,
                                          VkBool32 stencil_enable,VkStencilOpState front,
...
void VSetColorBlend(VGraphicsPipelineSpec* spec,
                    VkPipelineColorBlendAttachmentState* attachment_array,
                    u32 attachment_count,VkBool32 logicop_enable,
                    VkLogicOp logic_op,f32 blendconstants[4]){
    
    auto attachment =
        (VkPipelineColorBlendAttachmentState*)GetData(spec,
                                                      sizeof(VkPipelineColorBlendAttachmentState) * attachment_count);
    
    memcpy(attachment,attachment_array,attachment_count * sizeof(VkPipelineColorBlendAttachmentState));
    ...
    spec->colorblendstate.pAttachments = attachment;
    
    if(blendconstants){
        memcpy(spec->colorblendstate.blendConstants,blendconstants,sizeof(f32) * 4);  
    }
    
    spec->colorattachment_array = attachment;
    
}

//rename this to enable transparency
void VEnableColorBlendTransparency(VGraphicsPipelineSpec* spec,
                                   u32 colorattachment_index,
    ...
    spec->colorattachment_array[colorattachment_index] =
    {
...
void VEnableDynamicStateGraphicsPipelineSpec(VGraphicsPipelineSpec* spec,
    ...
    spec->dynamicstate =
        (VkPipelineDynamicStateCreateInfo*)GetData(spec,sizeof(VkPipelineDynamicStateCreateInfo));
    
    auto dynamic = (VkDynamicState*)GetData(spec,sizeof(VkDynamicState) * dynamic_count);
    
    memcpy(dynamic,dynamic_array,sizeof(VkDynamicState) * dynamic_count);
    ...
    spec->dynamicstate->pDynamicStates = dynamic;
}
...
void VCreateGraphicsPipelineArray(const  VDeviceContext* _restrict vdevice,VkPipelineCache cache,
                                  VGraphicsPipelineSpec* spec_array,u32 spec_count,
                                  VkPipeline* pipeline_array){
    
    VkGraphicsPipelineCreateInfo info[10];
    
    VkShaderModule shadermodule_array[60];
    u32 shadermodule_count = 0;
    
    VkPipelineShaderStageCreateInfo shaderinfo_array[60];
    u32 shaderinfo_count = 0;
    
    _kill("pipeline out of bounds\n",spec_count > 10);
    
    for(u32 i = 0; i < spec_count;i++){
        
        _kill("pipeline spec has a null layout\n",!spec_array[i].layout);
        _kill("pipeline spec has a null renderpass\n",!spec_array[i].renderpass);
        
        auto shaderinfo = &shaderinfo_array[shaderinfo_count];
        shaderinfo_count += spec_array[i].shaderdata_count;
        
        for(u32 j = 0; j < spec_array[i].shaderdata_count; j++){
            
            shadermodule_array[shadermodule_count] =
                VCreateShaderModule(vdevice->device,spec_array[i].shaderdata_array[j],
                                    spec_array[i].shaderdatasize_array[j],0);
            
            VkSpecializationInfo* specialization = 0;
            
            if(spec_array[i].shaderspecialization_array[j].mapEntryCount != 0){	
                specialization = &spec_array[i].shaderspecialization_array[j];
            }
            ...
            shaderinfo[j] = {
                ...
                0,
                0,
                spec_array[i].shaderstage_array[j],
                shadermodule_array[shadermodule_count],
                "main",
                specialization
            };
        ...
        info[i] =
        {
            ...
            0,
            spec_array[i].flags,
            spec_array[i].shaderdata_count,
            shaderinfo,
            &spec_array[i].vertexinput,
            &spec_array[i].assembly,
            spec_array[i].tessalationstate,
            &spec_array[i].viewport,
            &spec_array[i].raster,
            &spec_array[i].multisample,
            &spec_array[i].depthstencil,
            &spec_array[i].colorblendstate,
            spec_array[i].dynamicstate,
            spec_array[i].layout,
            spec_array[i].renderpass,
            spec_array[i].subpass_index,
            spec_array[i].parent_pipeline,
            spec_array[i].parentpipeline_index
        };
        
    }
    
    _vktest(vkCreateGraphicsPipelines(vdevice->device,cache,spec_count,info,global_allocator,
                                      pipeline_array));
...
void PushBackShaderPipelineSpec(VGraphicsPipelineSpec* spec,const s8* filepath,
    ...
    VPushBackShaderPipelineSpec(spec,shaderdata,shadersize,stage,specialization);
