...
void FileToTable(const s8* asset_list, AssetTable* out_table) {
    ...
    auto file = FOpenFile(asset_list, F_FLAG_READWRITE | F_FLAG_CREATE); //FileHandle
    ptrsize list_count;
...
void RemoveAssetFromList(u32 index, AssetTable* out_table) {
    
    if(index >= out_table->count){
        printf("WARNING: Index %d doesn't exist\n", index);
        return;
    }
    
    out_table->Remove(index);
}
...
s32 main(s32 argc,s8** argv){
        ...
        if (PHashString(argv[3]) == PHashString("-all")) {
            ...
            for (s32 i = 3; i < argc; i++) {
                ...
                if(PIsStringInt(argv[i])){
                    RemoveAssetFromList(atoi(argv[i]), &asset_table);
                }
    ...
    else if(PHashString(argv[2]) == PHashString("-cmp")){
